      ...
      <DialogContent className="max-w-lg">
        ...
        <motion.form initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} onSubmit={handleSubmit} className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-start text-xs text-muted-foreground p-2 bg-muted/20 rounded-md">
              <Info className="h-4 w-4 mr-2 mt-0.5 flex-shrink-0" />
              <span>If you link products, customers will select from this list. If left empty, customers must enter their Amazon Order ID for SP-API verification.</span>
            </div>
          </div>

          <div className="space-y-2">
            <Label htmlFor="promoMessage">Promotional Message</Label>
            <Textarea id="promoMessage" placeholder="e.g., Get a $5 gift card for your review!" value={formData.promo_message} onChange={(e) => handleInputChange('promo_message', e.target.value)} />
          </div>
          
          <div className="flex space-x-3 pt-4">
            <Button type="button" variant="outline" onClick={onClose} className="flex-1" disabled={uploading}>Cancel</Button>
            <Button type="submit" variant="accent" className="flex-1" disabled={uploading}>
